package com.mkc.sparkinjava.sparkSql;
...
public class TypedDataset {
	...
	public static void main(String[] args) throws Exception {
		...
		Dataset<Row> responseWithSelectedColumns = responses.select(col("country"),
				col("age_midpoint").as("ageMidPoint").cast("integer"), col("occupation"),
		...
		typedDataset.filter((FilterFunction<Response>) response -> response.getAgeMidPoint() != null
				&& response.getAgeMidPoint() < 20).show();
		...
		typedDataset.filter((FilterFunction<Response>) response -> response.getSalaryMidPoint() != null)
				.groupBy("country").agg(avg(SALARY_MIDPOINT), max(AGE_MIDPOINT)).show();

		System.out.println("=== Group by salary bucket ===");
		typedDataset
				.map((MapFunction<Response, Integer>) response -> response.getSalaryMidPoint() == null ? null
						: Math.round(response.getSalaryMidPoint() / 20000) * 20000, Encoders.INT())
				.withColumnRenamed("value", SALARY_MIDPOINT_BUCKET).groupBy(SALARY_MIDPOINT_BUCKET).count()
