package com.mkc.sparkinjava.advanced.broadcast;
...
public class UkMakerSpaces {
	...
	public static void main(String[] args) throws Exception {
		...
		JavaRDD<String> makerSpaceRdd = javaSparkContext.textFile("src/in/uk-makerspaces-identifiable-data.csv");

		JavaRDD<String> regions = makerSpaceRdd
				.filter(line -> !line.split(Utils.COMMA_DELIMITER, -1)[0].equals("Timestamp")).map(line -> {
	...
	private static Map<String, String> loadPostCodeMap() throws FileNotFoundException {
		Scanner postCode = new Scanner(new File("src/in/uk-postcode.csv"));
